...
for(x=0;x<6;x=x+1) begin
	for(y=0;y<3;y=y+1) begin
		wire G_clock_image;
		wire G_sleep_image = !(x == count_in_x && count_in_y == y);
		GATED_OR GATED_image (
			.CLOCK(clk),
			.SLEEP_CTRL(G_sleep_image&&cg_en),	// gated clock
			.RST_N(rst_n),
			.CLOCK_GATED(G_clock_image)
		);
		always@(posedge G_clock_image) begin
...
for(x=0;x<9;x=x+1) begin
	wire G_clock_kernel;
	wire G_sleep_kernel = !(count_in == x);
	GATED_OR GATED_kernel (
		.CLOCK(clk),
		.SLEEP_CTRL(G_sleep_kernel&&cg_en),	// gated clock
		.RST_N(rst_n),
		.CLOCK_GATED(G_clock_kernel)
	);
	always@(posedge G_clock_kernel) begin
		if(x == count_in)
			kernel[x] <= ker;
		else
			kernel[x] <= kernel[x];
...
for(x=0;x<4;x=x+1) begin
	wire G_clock_fc_weight;
	wire G_sleep_fc_weight = !(count_in == x);
	GATED_OR GATED_fc_weight (
		.CLOCK(clk),
		.SLEEP_CTRL(G_sleep_fc_weight&&cg_en),	// gated clock
		.RST_N(rst_n),
		.CLOCK_GATED(G_clock_fc_weight)
	);
	always@(posedge G_clock_fc_weight) begin
...
wire G_clock_conv;
wire G_sleep_conv = !(conv_in_valid);
GATED_OR GATED_conv (
	.CLOCK(clk),
...
wire G_clock_conv_valid;
wire G_sleep_conv_valid = !(conv_in_valid || conv_out_valid);
GATED_OR GATED_conv_valid (
	.CLOCK(clk),
	.SLEEP_CTRL(G_sleep_conv_valid&&cg_en),	// gated clock
	.RST_N(rst_n),
	.CLOCK_GATED(G_clock_conv_valid)
);

conv c1(
	.rst_n(rst_n),
	.clk(G_clock_conv),
	.clk_valid(G_clock_conv_valid),
...
// quantization
wire G_clock_quant_1;
wire G_sleep_quant_1 = !(conv_out_valid || conv_quantize_out_valid);
GATED_OR GATED_quant_1 (
	.CLOCK(clk),
	.SLEEP_CTRL(G_sleep_quant_1&&cg_en),	// gated clock
	.RST_N(rst_n),
	.CLOCK_GATED(G_clock_quant_1)
);
...
// max pool
wire G_clock_max_pool;
wire G_sleep_max_pool = !(conv_quantize_out_valid || max_pool_out_valid);
GATED_OR GATED_max_pool (
	.CLOCK(clk),
	.SLEEP_CTRL(G_sleep_max_pool&&cg_en),	// gated clock
	.RST_N(rst_n),
	.CLOCK_GATED(G_clock_max_pool)
);

max_pool m1(
	.rst_n(rst_n),
	.clk(G_clock_max_pool),
...
// fully connect
wire G_clock_fc;
wire G_sleep_fc = !(max_pool_out_valid || fc_out_valid);
GATED_OR GATED_fc (
	.CLOCK(clk),
	.SLEEP_CTRL(G_sleep_fc&&cg_en),	// gated clock
	.RST_N(rst_n),
	.CLOCK_GATED(G_clock_fc)
);
fully_connect fc1(
	.rst_n(rst_n),
	.clk(G_clock_fc),
...
// quantization 2
wire G_clock_quant_2;
wire G_sleep_quant_2 = !(fc_out_valid || fc_quantization_out_valid);
GATED_OR GATED_quant_2 (
	.CLOCK(clk),
	.SLEEP_CTRL(G_sleep_quant_2&&cg_en),	// gated clock
	.RST_N(rst_n),
	.CLOCK_GATED(G_clock_quant_2)
);
quatization_2 q2(
	.rst_n(rst_n),
	.clk(G_clock_quant_2),
...
// l1 distance
wire G_clock_l1;
wire G_sleep_l1 = !(fc_quantization_out_valid || l1_out_valid);
GATED_OR GATED_l1 (
	.CLOCK(clk),
	.SLEEP_CTRL(G_sleep_l1&&cg_en),	// gated clock
	.RST_N(rst_n),
	.CLOCK_GATED(G_clock_l1)
);
L1_distance l1(
	.rst_n(rst_n),
	.clk(G_clock_l1),
...
//output
wire G_clock_out;
wire G_sleep_out = !(l1_out_valid || out_valid);
GATED_OR GATED_out (
	.CLOCK(clk),
